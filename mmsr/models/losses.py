...
class BBL(nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, ksize=3, pad=0, stride=3, dist_norm='l2', criterion='l1', loss_weights=1e-4):
...
def gradient_penalty_loss(discriminator, real_data, fake_data, mask=None):
    ...
    alpha = real_data.new_tensor(torch.rand(batch_size, 1, 1, 1)).to('cuda:0')
