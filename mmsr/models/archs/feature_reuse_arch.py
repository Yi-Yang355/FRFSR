...
class TextureNet(nn.Module):
    ...
    def forward(self, x, sisr, pre_offset_flow_sim, img_ref_feat):
        """
        Args:
        ...
        upscale_restore = self.dyn_agg_restore(base, sisr, pre_offset_flow_sim, img_ref_feat)

        return upscale_restore
...
class DynamicAggregationRestoration(nn.Module):
    ...
    def __init__(self,
        ...
        self.ddf_block_1 = arch_util.make_layer(arch_util.ResidualDDFBlock, n_blocks//4, nf=ngf)
        ...
        self.ddf_block_2 = arch_util.make_layer(arch_util.ResidualDDFBlock, n_blocks//4, nf=ngf)
        ...
        self.ddf_block_3 = arch_util.make_layer(arch_util.ResidualDDFBlock, n_blocks//8, nf=ngf)
        ...
        self.ddf_block_4 = arch_util.make_layer(arch_util.ResidualDDFBlock, n_blocks//4, nf=ngf)
        ...
        self.ddf_block_5 = arch_util.make_layer(arch_util.ResidualDDFBlock, n_blocks//4, nf=ngf)
        ...
        self.high_q_conv2 = nn.Conv2d(128, 64, 3, 1, 1)
    ...
    def forward(self, base, sisr, pre_offset_flow_sim, img_ref_feat):
        ...
        x0_agg = torch.cat([sisr[0], x0], dim=1)
        x0_agg = self.lrelu(self.high_q_conv1(x0_agg))
        
        h = torch.cat([x0_agg, down_relu1_swapped_feat], 1)
        ...
        x_agg = torch.cat([sisr[0], x], dim=1)
        x_agg = self.lrelu(self.high_q_conv2(x_agg))
                      
        # large scale
        h = torch.cat([x_agg+x0, relu1_swapped_feat], 1)
        h = self.up_head_large(h)
        h = self.ddf_block_5(h) + x
        h_return = self.up_body_large(h) + x
